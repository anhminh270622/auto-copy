...
export default async function handler(req, res) {
    ...
    try {
        ...
        const adaptive = (info.streaming_data?.adaptive_formats || [])
            ...
            .map(f => {
                ...
                return {
                    ...
                    ext: f.mime_type?.includes('webm') ? 'webm'
                        : f.mime_type?.includes('audio/mp4') ? 'm4a' : 'mp4',
                    hasVideo: isVideo,
                    hasAudio: !isVideo,
                    size: formatBytes(f.content_length),
                };
        ...
        const videoOnly = adaptive.filter(f => f.hasVideo)
            .filter(f => f.ext === 'mp4')
            .sort((a, b) => (parseInt(b.quality) || 0) - (parseInt(a.quality) || 0));

        const audioOnly = adaptive.filter(f => f.hasAudio)
            .filter(f => f.ext === 'm4a')
            .sort((a, b) => (parseInt(b.quality) || 0) - (parseInt(a.quality) || 0));
